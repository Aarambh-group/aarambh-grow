    ...
    { label: "About Us", href: "/about" },
    {
        ...
        children: [
            ...
            },
            {
                ...
                description: "Loans, subsidies & funding support",
                icon: Banknote
            },
            {
            ...
            },
            {
                ...
                description: "Process, automation & business consulting",
                icon: Workflow
            },
            {
                label: "Legal & CA Services",
                href: "/services/legal-ca",
                description: "GST, ITR, ROC & compliance",
                icon: Scale
            }
        ],
    },
    ...
    return (
        <div className="group relative py-5 flex items-center">
            ...
            <Link
                ...
                className={`relative z-10 flex items-center gap-1 px-2.5 py-1.5 text-[13px] font-semibold transition-colors duration-300 ease-out select-none whitespace-nowrap hover:text-[#F26522] ${isActive ? "text-[#F26522]" : "text-[#03254C]"
                    }`}
            >
                {/* Text Layer */}
                <span className="relative z-10 block transition-transform duration-300 ease-out group-hover:-translate-y-0.5">
                ...
                {/* ANIMATED UNDERLINE EFFECT */}
                <span
                    className={`absolute bottom-1 left-2.5 right-2.5 h-[2px] transition-transform duration-300 ease-out origin-left rounded-full group-hover:scale-x-100 ${isActive ? "scale-x-100" : "scale-x-0"
                        }`}
                    style={{
                        background: "linear-gradient(90deg, #F26522 0%, #157327 100%)"
                    }}
            ...
            {/* Premium Submenu / Mega Menu */}
            {hasChildren && (
                <div className={`invisible absolute top-full z-20 pt-2 opacity-0 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:visible group-hover:opacity-100 group-hover:translate-y-0 translate-y-3 pointer-events-none group-hover:pointer-events-auto ${isMegaMenu
                    ...
                    {isMegaMenu ? (
                        // MEGA MENU LAYOUT
                        <div className="overflow-hidden rounded-lg border border-slate-100/80 bg-white/95 shadow-[0_20px_50px_-12px_rgba(3,37,76,0.12)] backdrop-blur-xl transition-all duration-300 flex">
                            {/* Links Grid Section */}
                            <div className="w-2/3 p-5">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="h-4 w-1 rounded-full bg-[#F26522]"></span>
                                    <h3 className="text-xs font-bold uppercase tracking-wider text-[#03254C]">Explore Services</h3>
                                </div>
                                <p className="text-[11px] text-slate-500 mb-4 ml-3">Complete business solutions under one roof</p>
                                        ...
                                        {link.children.slice(0, 3).map((child) => {
                                            ...
                                            return (
                                                <Link
                                                    key={child.label}
                                                    href={child.href}
                                                    className="group/item flex flex-col gap-0.5 p-2 rounded-lg transition-all duration-200 hover:bg-[#F26522]/5"
                                                >
                                                    <div className="flex items-center gap-2.5">
                                                        {Icon && (
                                                            <Icon className="h-[18px] w-[18px] text-[#03254C]/60 transition-colors duration-200 group-hover/item:text-[#F26522] shrink-0" strokeWidth={1.5} />
                                                        )}
                            ...
                            <div className="w-1/3 bg-gradient-to-br from-[#03254C] to-[#062a57] p-6 flex flex-col justify-center text-white relative overflow-hidden border-l border-slate-100/10">
                                <div className="absolute -top-6 -right-6 w-24 h-24 bg-[#F26522]/20 rounded-full blur-2xl"></div>
                                <div className="absolute -bottom-6 -left-6 w-20 h-20 bg-[#157327]/20 rounded-full blur-2xl"></div>

                                <div className="relative z-10">
                                    <h4 className="text-sm font-bold mb-2 text-white">Need Help Deciding?</h4>
                                    <p className="text-[11px] text-slate-300 leading-relaxed mb-4">
                                        Our experts are here to guide you through the best solutions for your business growth.
                                    </p>
                                    <Link
                                        href="/contact"
                        ...
                        // ORIGINAL SIMPLE DROPDOWN LAYOUT
                        <div className="overflow-hidden rounded-lg border border-slate-100/80 bg-white/95 p-2 shadow-[0_20px_50px_-12px_rgba(3,37,76,0.12)] backdrop-blur-xl transition-all duration-300">
                            {link.children.map((child, idx) => (
                                <Link
                                    key={child.label}
                                    href={child.href}
                                    style={{ transitionDelay: `${idx * 25}ms` }}
                                    className="group/item flex items-center justify-between rounded-lg px-3 py-2.5 text-xs font-semibold text-[#03254C]/80 transition-all duration-200 hover:bg-[#F26522]/10 hover:text-[#F26522] hover:translate-x-1"
                                >
                                    <span className="transition-transform duration-200 group-hover/item:translate-x-0.5">
                                        {child.label}
                                    </span>
        ...
        <div className="border-b border-slate-100/60 last:border-none">
            <div className="flex w-full items-center justify-between py-0.5">
                <Link
                    ...
                    onClick={onNavigate}
                    aria-current={isActive ? "page" : undefined}
                    className={`flex-grow py-2.5 text-[13px] font-semibold tracking-wide transition-colors hover:text-[#F26522] ${isActive ? "text-[#F26522]" : "text-[#03254C]"
                        }`}
                >
                    {link.label}
                </Link>
                            ...
                            {link.children?.map((child) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        href={child.href}
                                        onClick={onNavigate}
                                        className="group/m flex items-center gap-2.5 text-xs font-semibold text-slate-500 py-1.5 px-2 rounded-lg hover:text-[#F26522] hover:bg-slate-50 transition-all duration-150 active:scale-98"
                                    >
                                        {Icon && (
                                            <Icon className="h-[18px] w-[18px] text-[#03254C]/50 group-hover/m:text-[#F26522] transition-colors shrink-0" strokeWidth={1.5} />
                                        )}
                                        {child.label}
                                    </Link>
                                );
